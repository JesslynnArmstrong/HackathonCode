...
#region methods
        ...
        public void Initialize(bool resetRange = false)
        {
            ...
            {
                int key = 0;
                if (_sourceType == SOURCE_TYPE.Network && _sequenceName.Substring(0,4) != "http")
                {
                    Debug.LogError("Plugin 4DS: Source Type is Network but URL does not start with http://");
                    return;
                }
            ...
            if (_dataSource.TextureFormat == TextureFormat.PVRTC_RGB2 )  //pvrtc2 is 2bpp
                pixelBufferSize /= 2;
            if (_dataSource.TextureFormat == TextureFormat.ASTC_8x8)
            {
                int blockSize = 8;
